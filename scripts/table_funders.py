...
def generate_funder_latex_table(
    ...
    # Color gradient is anchored on the headline (observed) rate so the
    # observed column drives the visual signal; the corrected column is
    # shaded on the same scale to make the modeled correction visible
    # without overtaking the headline. (#20)
    ...
    if has_correction:
        caption = (
            ...
            r"applies journal-level head-to-head PDF vs.\ XML correction factors "
            r"to the XML-only portion; it is informative about the magnitude of "
            r"XML undercount but assumes the head-to-head subset is representative "
            r"of XML-only articles within each journal. "
            r"Cell shading: Total Pubs uses a blue-to-red gradient on log scale; "
            r"both \% OD columns share a linear blue-to-red gradient anchored on "
